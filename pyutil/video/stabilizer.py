...
from pyutil.video.vid import Vid 
...
class Stabilizer: 
    def __init__(
        ...
        vid.output_size = (int(original_output[0]/downscaling_factor), int(original_output[1]//downscaling_factor))
    ...
    def __next__(self): 
        ...
        m = np.array([
            [np.cos(dr), -np.sin(dr), dx],
            [np.sin(dr),  np.cos(dr), dy]
        ])


        image_stab = cv2.warpAffine(image, m, self.vid.wh)

        return self.post_process(image_stab)
